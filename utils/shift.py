...
class Shift:
    ...
    def __init__(self, data, depot, cluster_class, graph_class, start_date=None):
        ...
        self.depot = depot
        self.start_date = data.index[-1] if start_date is None else pd.to_datetime(
            start_date)
    ...
    def clusterize(self, columns, max_size=175, kwargs={}):
        ...
        if(self.cluster_class is None):
            ...
            self.data.loc[:, 'cluster'] = cluster_method.fit_predict(
                X.values.tolist())
        return self.data
    ...
    def build_graphs(self, columns, kwargs={}):
        ...
        if(self.cluster_class is not None):
            ...
            for c in clusters:
                ...
                nodes = [[x, y] for x, y in zip(
                    current_data['latitude'], current_data['longitude'])]
                self.graphs.append(self.graph_class(
                    nodes, start_node=self.depot, end_node=self.depot, **kwargs))
        else:
            nodes = [[x, y]
                     for x, y in zip(self.data['latitude'], self.data['longitude'])]
    ...
    def get_timetables(self, distance_function, speed=30, emp_time=60):
        ...
        for route in self.routes:
            ...
            timetable.append(
                self.start_date + timedelta(seconds=distance_function(route[0], route[1])/speed*3600))
            for i in range(2, len(route)):
                d = distance_function(route[i], route[i-1])
                current_time = (d/speed)*3600 + emp_time
                timetable.append(
                    timetable[i-1]+timedelta(seconds=current_time))
